---
title: 源码系列(1)——ActivityThread的理解
date: 2020-06-15 14:19:08
tags: [Android, ActivityThread, 源码]
summary: UI绘制流程之ActivityThread的理解
---
...
用于创建`Activity`，`mStackSupervisor`是`AMS`的成员变量，为`Activity`堆栈管理辅助类实例，该方法最终会调用`ApplicationThread`类的`scheduleLaunchActivity`方法，该方法向`ActivityThread`的消息队列发送创建`Activity`的消息，最终在`ActivityThread`中完成创建`Activity`的操作。
    ...
    ```
    其中，`app.thread.scheduleLaunchActivity(...);`会调用`ApplicationThread`的`scheduleLaunchActivity`方法，该方法会向`ActivityThread`发送`LAUNCH_ACTIVITY`信息，用于启动一个`Activity`，该消息的处理会调用`ActivityThread`的`handleLaunchActivity`方法，最终启动一个`Activity`
